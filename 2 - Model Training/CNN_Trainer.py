...
if __name__ == "__main__":
    ...
    try:
        ...
        # respect config default for dynamic weight if CLI not provided
    ...
    # Validate labels vs selected loss to avoid common mismatch (one-hot vs sparse)
    try:
        import numpy as _np
        ...
        if (not is_onehot) and isinstance(loss_fn, str) and loss_fn == 'categorical_crossentropy':
            raise RuntimeError("Label format appears integer/sparse but loss is 'categorical_crossentropy'. Use sparse labels or convert labels to one-hot.")
    ...
    try:
        ...
        if getattr(_cfg, 'SAVE_TRAINING_METADATA', False):
            ...
            meta_path = _cfg.RESULTS_DIR / 'cnn_training_metadata.json'
