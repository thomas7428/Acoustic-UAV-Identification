...
def save_predictions(dataset_path: Path, json_path: str, limit: int = None):
    ...
    threshold = config.MODEL_THRESHOLDS.get('CRNN', 0.5)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate CRNN on dataset_test')
    ...
    labels, preds_bin, scores, filenames = save_predictions(DATASET_PATH, config.CRNN_PREDICTIONS_PATH_STR, limit=limit)
    perf = performance_calcs(config.CRNN_SCORES_PATH_STR, labels, preds_bin)

    print(colored(f"CRNN model performance scores have been saved to {config.CRNN_SCORES_PATH_STR}.", "green"))
