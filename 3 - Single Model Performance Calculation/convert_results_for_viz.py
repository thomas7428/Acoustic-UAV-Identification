...
def convert_scores_to_accuracy_format(scores_path, output_path, model_name):
    ...
    # Extract values (they are stored as single-item lists)
    TP = scores['TP'][0]
    FN = scores['FN'][0]
    TN = scores['TN'][0]
    FP = scores['FP'][0]
...
def main():
    ...
    print("=" * 60)
    print(f"Conversion complete: {converted_count}/3 models converted")
    print("=" * 60)
    ...
    if converted_count > 0:
        ...
        print("  - CNN_and_CRNN_Performance_Calcs.py (for CNN)")
        print("  - RNN_Performance_Calcs.py (for RNN)")
        print("  - CNN_and_CRNN_Performance_Calcs.py (for CRNN, modify SAVED_MODEL_PATH)")
